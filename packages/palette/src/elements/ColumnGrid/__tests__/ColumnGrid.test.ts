import { calculateGridColumn } from "../ColumnGrid"

describe("calculateGridColumn", () => {
  it("returns the correct value for a single start", () => {
    expect(calculateGridColumn({ start: 4 })).toEqual(["4 / -1"])
  })

  it("returns the correct value for a single span", () => {
    expect(calculateGridColumn({ span: 4 })).toEqual(["span 4"])
  })
  ...
  it("returns the correct value for simple inputs of both", () => {
    expect(calculateGridColumn({ start: 1, span: 2 })).toEqual(["1 / span 2"])
  })
